...
export default function ChatAluno() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      await supabase
        .from('mensagens')
        .update({ lida: true })
        .eq('remetente_id', profile.personal_id)
  ...
  const carregarMensagens = async (alunoId: string, personalId: string) => {
    const { data } = await supabase
      .from('mensagens')
      .select('*')
      .or(`and(remetente_id.eq.${alunoId},destinatario_id.eq.${personalId}),and(remetente_id.eq.${personalId},destinatario_id.eq.${alunoId})`)
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col font-sans">
      ...
      <header className="border-b border-gray-800 p-4 flex items-center gap-4 bg-black sticky top-0 z-10">
        <button onClick={() => router.push('/home')} className="text-gray-500 hover:text-white transition-colors">
        ...
        <div>
          <p className="font-bold text-sm">{personal?.full_name}</p>
          <p className="text-green-400 text-[10px] font-black uppercase tracking-widest">Seu Personal</p>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-3" style={{maxWidth:700, width:'100%', margin:'0 auto'}}>
        {mensagens.length === 0 && (
          <div className="text-center py-20 text-gray-600">
            ...
            <p className="text-xs mt-2">Inicie a conversa com seu personal!</p>
          </div>
        )}
        {mensagens.map((msg) => {
